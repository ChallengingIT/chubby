...
public interface IntervistaRepository extends JpaRepository<Intervista, Integer> {
    ...
    @Query(value= """
                ...
                left join owner o on io.id_owner = o.id
                left join users u on o.nome = u.nome and o.cognome = u.cognome
                where u.username = ?1
                and WEEK(DATE(i.ora_aggiornamento)) = WEEK(DATE(now() + INTERVAL 1 WEEK ))
    ...
    @Query(value= """
                ...
                left join users u on o.nome = u.nome and o.cognome = u.cognome
                where u.username = ?1
                and WEEK(DATE(i.ora_aggiornamento)) = WEEK(DATE(now() + INTERVAL ?2 + 1 WEEK ))
                order by i.ora_aggiornamento desc
        """, nativeQuery=true)
    ...
    @Query(value= """
                ...
                left join tipologia_intervista ti on (i.id = ti.id_intervista)
                left join owner o on io.id_owner = o.id
                where WEEK(DATE(i.ora_aggiornamento)) = WEEK(DATE(now() + INTERVAL ?1 + 1 WEEK))
                order by i.ora_aggiornamento desc
        """, nativeQuery=true)
